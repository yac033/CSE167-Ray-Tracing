...
void Scene::init(void){
    ...
    node["bunny2"] -> modeltransforms.push_back(scale(vec3(0.5f)) * translate(vec3(0.0f,1.0f,0.0f)));
    ...
    node["world"] -> childtransforms.push_back( translate(vec3(-2.7f,0.0f,0.0f)) * rotate( 90.0f*float(M_PI)/180.0f, vec3(0.0f, 1.0f, 0.0f) ));
    node["world"] -> childnodes.push_back( node["teapot1"] );
    node["world"] -> childtransforms.push_back( translate(vec3(-2.0f,0.0f,1.0f)) * scale(vec3(0.5f)) );
    // Put a camera
    camera = new Camera;
